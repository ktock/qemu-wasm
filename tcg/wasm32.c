    ...
    CPUArchState *env, uint64_t taddr, MemOp mop, uint64_t* ptr, bool is_ld)
{
    ...
    uint64_t table = *(uint64_t *)((int)env + table_ofs);
    uint64_t mask = *(uint64_t *)((int)env + mask_ofs);
    ...
    if (c_addr == target) {
        return taddr + *(uint64_t *)(entry + add_off);
    }
    return 0;
}
...
static void init_wasm32(void)
{
    ...
    ctx.stack128 = g_malloc(TCG_STATIC_CALL_ARGS_SIZE + TCG_STATIC_FRAME_SIZE);
